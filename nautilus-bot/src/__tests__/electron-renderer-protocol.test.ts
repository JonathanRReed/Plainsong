...
describe("packaged renderer protocol", () => {
  ...
  it("validates playback tokens securely", () => {
    ...
    expect(playbackTokenFromUrl(`plainsong://playback./${validToken}`)).toBe(validToken);
    expect(playbackTokenFromUrl(`plainsong://user:pass@playback/${validToken}`)).toBe(null);
    expect(playbackTokenFromUrl(`plainsong://playback:8080/${validToken}`)).toBe(null);
  });
