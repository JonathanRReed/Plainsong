...
if (
  isFixtureBenchmarkRun(benchmarkBaseline) ||
  isFixtureBenchmarkRun(benchmarkMacos) ||
  isFixtureBenchmarkRun(benchmarkWindows)
) {
  blockers.push({
    ...
    reason:
      "Benchmark gate artifacts are still local or fixture-tagged runs; packaged dictation benchmark evidence is still missing.",
  });
}
...
if (qaSummary.pass < qaSummary.total) {
  blockers.push({
    gate: "packaged-qa-matrix",
    status: "BLOCKED",
    evidence: "artifacts/packaged-qa-evidence-bundle.json",
    reason: `Packaged QA matrix remains ${qaSummary.blocked} BLOCKED / ${qaSummary.pass} PASS.`,
  });
}
