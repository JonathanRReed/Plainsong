//! Speaker-turn normalization shared by the end-to-end diarization backends
//! (speakrs / pyannote community-1 and Nemotron 3 Diarization).
//!
//! Both report hard speaker turns with their own opaque labels. This module
//! turns those into Plainsong segments with one set of rules: stable `S1..Sn`
//! ids by first appearance, breath gaps merged, flickers dropped, and
//! uncovered audio left **unattributed** rather than defaulted to `S1`.
//! Compiled into every test build so the rules stay tested even when neither
//! backend feature is on.

use super::{Speaker, SpeakerSegment};

/// Neither backend reports a usable per-turn confidence: their output is a
/// hard speaker assignment per frame, not a posterior. Reporting a made-up number
...
pub(crate) const MODEL_SEGMENT_CONFIDENCE: f64 = 0.0;

/// Two turns of the same speaker separated by less than this are one turn.
/// Both backends emit frame-resolution turns (~16 ms for speakrs at its default
/// 1 s segmentation step, 10 ms for Nemotron), so a speaker who pauses for breath comes back as two
...
/// maximum overlap.
pub(crate) const MIN_TURN_SECONDS: f64 = 0.2;

/// A speaker turn exactly as a backend reports it, before this module imposes
...
pub(crate) struct RawTurn {
    pub(crate) start: f64,
    pub(crate) end: f64,
    /// The backend's own label, e.g. speakrs's `SPEAKER_00`. Opaque here: only its identity
...
pub(crate) fn normalize_turns(turns: &[RawTurn], duration: f64) -> Vec<SpeakerSegment> {
    ...
    // Merge on the backend's own label, against the most recent turn of that
    // speaker wherever it sits -- not only the last turn emitted. Both
    // pyannote's powerset decoding and Sortformer report overlapping speech,
    // so A-B-A with A's two turns overlapping is ordinary output; considering only the last turn left two
    ...
    merged
        ...
        .map(|turn| {
            ...
            SpeakerSegment {
                ...
                confidence: MODEL_SEGMENT_CONFIDENCE,
            }
        })
        .collect()
}
...
pub(crate) fn speakers_for(segments: &[SpeakerSegment]) -> Vec<Speaker> {
    const COLORS: [&str; 6] = [
