...
const VOXTRAL_SMALL_MODEL_ID: &str = "voxtral-small";
...
fn sanitize_voxtral_mode(model_id: Option<&str>) -> VoxtralMode {
    match model_id.unwrap_or(VOXTRAL_LOCAL_MODEL_ID).trim() {
        VOXTRAL_CLOUD_MODEL_ID | VOXTRAL_SMALL_MODEL_ID => VoxtralMode::Cloud,
...
impl VoxtralProvider {
    ...
    fn api_model(&self) -> &'static str {
        match self.selected_model_id() {
            VOXTRAL_SMALL_MODEL_ID => "voxtral-small-latest",
            _ => VOXTRAL_API_MODEL,
        }
    }
    ...
    async fn transcribe_cloud(&self, audio_data: &[u8]) -> Result<TranscriptionResult> {
        ...
        Ok(TranscriptionResult {
            ...
            model_name: format!(
                "Voxtral {} ({})",
                if self.selected_model_id() == VOXTRAL_SMALL_MODEL_ID {
                    "Small"
                } else {
                    "Mini"
                },
...
impl AsrProvider for VoxtralProvider {
    ...
    fn model_info(&self) -> ModelInfo {
        let is_small = self.selected_model_id() == VOXTRAL_SMALL_MODEL_ID;
        ModelInfo {
            name: if is_small {
                "Voxtral Small 24B".to_string()
            } else {
                "Voxtral Mini 4B".to_string()
            },
            version: if is_small {
                "latest".to_string()
            } else {
            ...
            size_mb: if is_small { 48000.0 } else { 8192.0 },
            parameters: if is_small {
                "24B cloud"
            } else {
                "4B local / cloud"
            }
            ...
            .map(String::from)
            .collect(),
            word_error_rate: if is_small { Some(2.5) } else { None },
