...
fn check_os_status(os_status: OSStatus) -> Result<(), Error> {
    // kAudioDevicePermissionsError ('!hog') is returned by Core Audio process
    // taps when Screen & System Audio Recording access is denied. coreaudio-rs
    // 0.14.2 does not name that status, so without this guard CPAL reports an
    // opaque BackendError and callers cannot offer the correct recovery path.
    const AUDIO_DEVICE_PERMISSIONS_ERROR: OSStatus = 0x2168_6f67;
    if os_status == AUDIO_DEVICE_PERMISSIONS_ERROR {
        return Err(Error::with_message(
            ErrorKind::PermissionDenied,
            "Core Audio system-capture permission was denied",
        ));
    }
