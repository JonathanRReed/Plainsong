...
impl Device {
        ...
        E: FnMut(Error) + Send + 'static,
    {
        ...
        // A Device returned by default_output_device() is an explicit request to
        // capture output through a process tap. Never re-probe supports_input()
        // and reinterpret a duplex output as a physical input: that second probe
        // can disagree with selection after a transient property-query failure.
        let mut audio_unit = if self.is_default_output || !self.supports_input() {
        ...
        let monitor: Box<dyn Monitor> = if self.is_default_output {
            // Process taps are bound to the current default output. Report a
            // route change from Core Audio's property-listener thread instead of
            // forcing callers to poll device properties on their audio thread.
