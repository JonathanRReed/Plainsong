...
type CreateProcessTap =
    unsafe extern "C-unwind" fn(Option<&CATapDescription>, *mut AudioObjectID) -> i32;
type DestroyProcessTap = unsafe extern "C-unwind" fn(AudioObjectID) -> i32;

unsafe fn dynamic_core_audio_symbol<T>(name: &'static CStr) -> Option<T>
where
    T: Copy,
{
...
fn create_process_tap_function() -> Option<CreateProcessTap> {
    static FUNCTION: OnceLock<Option<CreateProcessTap>> = OnceLock::new();
    *FUNCTION.get_or_init(|| unsafe {
        dynamic_core_audio_symbol::<CreateProcessTap>(c"AudioHardwareCreateProcessTap")
    })
}

fn destroy_process_tap_function() -> Option<DestroyProcessTap> {
    static FUNCTION: OnceLock<Option<DestroyProcessTap>> = OnceLock::new();
    *FUNCTION.get_or_init(|| unsafe {
        dynamic_core_audio_symbol::<DestroyProcessTap>(c"AudioHardwareDestroyProcessTap")
    })
}
...
fn create_process_tap(
    ...
    let function = create_process_tap_function().ok_or_else(|| {
        Error::with_message(
            ErrorKind::UnsupportedOperation,
            "Core Audio process taps require macOS 14.2 or later",
        )
    })?;
    let status = unsafe { function(Some(description), out_tap_id) };
    check_os_status(status)
}

unsafe fn destroy_process_tap(tap_id: AudioObjectID) {
    if let Some(function) = destroy_process_tap_function() {
        let _status = function(tap_id);
    }
}
...
impl LoopbackDevice {
    ...
    pub fn from_device(device: &Device) -> Result<Self, Error> {
        ...
        let tap_obj_id = unsafe {
            create_process_tap(tap_desc.as_ref(), tap_obj_id.as_mut_ptr())?;
