...
| BR-002 | CP benchmark gate artifacts remain incomplete: local fixture-driven baseline + macOS JSON now exist, but `docs/evals/benchmark-run-latest-windows.json` is still missing and packaged dictation evidence is still absent | High | `artifacts/benchmark-gates-macos.json`, `artifacts/benchmark-gates-windows.blocked.md`, `docs/evals/dictation-parity-launch-scorecard.md` | Produce the Windows benchmark JSON artifact, capture packaged macOS/Windows dictation evidence, update `docs/dictation-app-compatibility-matrix.md`, close blocked apps in `docs/dictation-blocked-app-register.md`, and re-run `scripts/verify-benchmark-gates.mjs` for macOS and Windows. |
